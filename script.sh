...
function install_ss_panel_mod_UIm(){
	...
	wget -c --no-check-certificate https://raw.githubusercontent.com/EthanLuoo/sspanel-v3-mod-backup/master/lnmp1.5.zip && unzip lnmp1.5.zip && rm -rf lnmp1.5.zip && cd lnmp1.5 && chmod +x install.sh && ./install.sh lnmp
	cd /home/wwwroot/
	...
	#克隆项目
	git clone https://github.com/EthanLuoo/sspanel-v3-mod-backup-resource.git tmp && mv tmp/.git . && rm -rf tmp && git reset --hard
	#复制配置文件
	...
	#下载配置文件
	wget -N -P  /usr/local/nginx/conf/ --no-check-certificate "https://raw.githubusercontent.com/EthanLuoo/sspanel-v3-mod-backup/master/nginx.conf"
	wget -N -P /usr/local/php/etc/ --no-check-certificate "https://raw.githubusercontent.com/EthanLuoo/sspanel-v3-mod-backup/master/php.ini"
	#开启scandir()函数
...
EOF
	...
	wget -N -P  /home/wwwroot/default/vendor/composer --no-check-certificate "https://raw.githubusercontent.com/EthanLuoo/sspanel-v3-mod-backup/master/autoload_classmap.php"
...
function Libtest(){
	...
	if [ "$libAddr" == "$GIT" ];then
		libAddr='https://raw.githubusercontent.com/EthanLuoo/sspanel-v3-mod-backup/master/libsodium-1.0.13.tar.gz'
	else
...
function install_node(){
	...
	wget -N -P  /etc/ --no-check-certificate  https://raw.githubusercontent.com/EthanLuoo/sspanel-v3-mod-backup/master/supervisord.conf	
	supervisord
	#iptables
	iptables -F
	iptables -X  
...
function NEW_NODE(){
     wget -N --no-check-certificate  https://raw.githubusercontent.com/EthanLuoo/sspanel-v3-mod-backup/master/node.sh && bash node.sh
}
